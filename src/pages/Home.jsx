...
import { Eye, Palette, X, Zap, Globe } from "lucide-react";
...
export default function Home() {
  ...
  // Hands the URL to the browser shell, which fetches it through the Render
  // proxy backend. No browser-automation API and no frontend-side page fetch,
  // so navigation starts immediately instead of waiting on a cloud session.
  const loadPage = useCallback((rawUrl) => {
      ...
      <main className="relative z-10 flex-1 flex flex-col min-h-0">
        <div className="flex-1 flex flex-col items-center justify-center gap-8 px-4 sm:px-6 py-8 overflow-y-auto">
          ...
          <div className="w-full max-w-xl vp-fade-up" style={{ animationDelay: "0.08s" }}>
            <VelocitySearch value={query} onChange={setQuery} onSubmit={() => loadPage(query)} loading={false} />
          </div>
